...
// alert("Hi");
	...
	// SFscreenshift.style('z-index', '1032', 'collapsed');
	// SFscreenshift.style('z-index', '-1', 'banner');
	// SFscreenshift.anchor('#site-navbar-container');
	
	// SFscreenshift.anchor('.drawer-open #site-navbar-container');
	if (!window._spads) {
	  window._spads = {};
	}
	var adhesionBanner;
	...
	// SFscreenshift.elementObserve('#tncms-region-mobile-nav-top-left-region');
	SFscreenshift.changeObserve('html', function() {
		var observeEl = document.querySelector('html');
		if (observeEl.classList.contains('drawer-open')) {
			SFscreenshift.style('display', 'none', 'banner');  

			// not collapsed
			// SFscreenshift.collapsedPositionHandler(0);
		} else {
			//collapsed
			SFscreenshift.style('display', 'block', 'banner');  

			// SFscreenshift.collapsedPositionHandler(-38);
		}
	});


	_spads.formats['254'].callbacks.adEvent.push(function (id, type) {
		// navMenu = topWindow.document.querySelector('.drawer-open');
		// if(navMenu){
			// navMenu.style['z-index'] = '';
			// navMenu.setAttribute('style','z-index: ');
		  //   alert("Nav men/u found");
		//   SFscreenshift.style('display', 'none', 'banner');  
		// }
	  // }
		...
		// if (type == "exp") {
			/*
		  adhesionBanner = topWindow.document.querySelector("#site-navbar-container");
		  if (adhesionBanner) {
			adhesionBanner.style['display'] = 'auto';
			adhesionBanner.setAttribute('style', 'display:auto');
			adhesionBanner.style['position'] = 'auto';
			adhesionBanner.setAttribute('style', 'position:auto');
			adhesionBanner.style['visibility'] = 'auto';
			adhesionBanner.setAttribute('style', 'visibility:auto');
		  }
		  */
	  });
